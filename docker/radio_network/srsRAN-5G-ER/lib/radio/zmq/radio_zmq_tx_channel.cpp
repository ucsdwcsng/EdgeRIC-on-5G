 ...
 *
 */
#include "../../edgeric/edgeric.h" // Ushasi
#include <fstream>
#include <iostream>
...
void radio_zmq_tx_channel::receive_request()
{
  ...
  if (socket_type == ZMQ_REP) {
    ...
    if (n > 0) {
      ...
      // std::ofstream logfile("log_iq.txt", std::ios_base::app); // Open log file in append mode
      // if (logfile.is_open()) {
      //     logfile << "Socket received request." << std::endl;
      // }
      // logfile.close();
...
//   std::ofstream logfile("log_iq.txt", std::ios_base::app); // Open log file in append mode
//       if (logfile.is_open()) {
//           logfile << "TTI: " << edgeric::tti_cnt << ", number of samples: " << count << std::endl;
//       }
//   logfile.close();
...
//   std::ofstream logfile("log_iq.txt", std::ios_base::app); // Open log file in append mode
//       if (logfile.is_open()) {
//           logfile << "Socket sent samples: " << count << std::endl;
//       }
//       logfile.close();
...
void radio_zmq_tx_channel::send_response()
{
  ...
  // Open the log file only once
  // uint32_t count_tti = edgeric::tti_cnt;
  // std::ofstream logfile("log_iq.txt", std::ios_base::app); // Open log file in append mode
  // if (logfile.is_open()) {
  //     logfile << "TTI: " << count_tti << ", number of samples: " << count << ", bytes: " << nbytes << std::endl;
  // }
  ...
  //     logfile << "Socket sent samples: " << count << std::endl;
  // }
  // logfile.close();  // Close the log file here
  // If successful, transition to wait for data.
  state_fsm.data_sent();
}
...
void radio_zmq_tx_channel::run_async()
{
  ...
  if (!state_fsm.has_pending_request()) {
    receive_request();
    // if (logfile.is_open()) {
    //     logfile << "TTI: " << count_tti << ", Received Request" << std::endl;
    // }
  } else {
    send_response();
    // if (logfile.is_open()) {
    //     logfile << "TTI: " << count_tti << ", Sent Response" << std::endl;
    // }
  }
  ...
  if (state_fsm.has_wait_expired()) {
    ...
    // if (logfile.is_open()) {
    //     logfile << "TTI: " << count_tti << ", Waiting for " << (state_fsm.has_pending_request() ? "data" : "request") << "." << std::endl;
    // }
    // logfile.close();
